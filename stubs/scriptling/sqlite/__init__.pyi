"""
SQLite plugin — embedded relational database (pure Go).

The database file must fall inside the host's allowed paths;
...
def connect(path: str = ..., timeout_ms: int = 5000) -> Connection:
    """Open a SQLite database file and return a Connection.

    Parameters:
        path: database file path, or ``":memory:"`` (the default) for a
            private in-memory database
        timeout_ms: how long writers wait for a lock before failing
    """
    ...
