...
class Connection:
    ...
    def begin(self) -> Transaction:
        ...
        keeps them or rollback() discards them. ``?`` placeholders keep
        working on PostgreSQL (translated to ``$n``). Connection-level
        calls while the transaction is open run on separate pooled
        connections and do not see its uncommitted changes.
        """
        ...
